...
void setup() 
{
  ...
  for(int s = 0; s < N; s++)
  {
    ...
    check_for_prime[s].fields_num = FIELDS_NUM;
  ...
  template_prime.fields_num = FIELDS_NUM;
  ...
  template_not_prime.fields_num = FIELDS_NUM;
...
void loop() 
{
  ...
  while( 1 )
  {
    ...
    // Getting results and if result is valid increasing received tuple's counter
    ...
    // Breaking in case of getting required number of results
    if ( counter == N )
        break;
  ...
  counter = 0;          // Reseting received tuple's counter
